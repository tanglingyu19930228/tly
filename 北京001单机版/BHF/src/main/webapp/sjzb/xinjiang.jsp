...
<table class="implementPlanT">
	<c:if test="${partTag==1 }">
		<caption>新疆边海防基础设施建设任务统筹规划</caption>
	</c:if>
	<c:if test="${partTag==2 }">
		<caption>新疆${year }年度边海防基础设施建设任务</caption>
	</c:if>
	<thead>
		<tr>
	...
	<tbody>
		...
		<tr>
			<td rowspan="${data.lzbjssRows }">拦阻报警设施</td>
		...
		<!-- 边境卡口监控站 没有数据 -->
		<tr>
			<td rowspan="${data.jkzBjkkRows }">边境卡口监控站</td>
			<td rowspan="${data.jkzBjkkXjRows }">新建</td>
			<c:if test="${empty data.jkzBjkkXj }">
				<td></td>
				<td></td>
				<td></td>
				<td></td>
			...
			<c:forEach items="${data.jkzBjkkXj }" var="item" varStatus="s">
				<c:if test="${s.index eq 0 }">
					<td>${s.index+1 }</td>
					<td>${item.fzdd }</td>
					<td></td>
			...
			<td rowspan="${data.jkzBjkkXjRows }"></td>
			<td rowspan="${data.jkzBjkkXjRows }"></td>
			<td rowspan="${data.jkzBjkkXjRows }"></td>
			<td rowspan="${data.jkzBjkkXjRows }"></td>
		</tr>
