//根据建设状态统计显示的饼状图
var peiColor=["#78BCBF","#7289AB","#92CA8D","#E59D87","#E05552","#F49F42"];
...
var jszt_pei=new Array();
var jszt_data=new Array();
...
$(function(){
	var jsztPie=$("#jsztPie").val().replace("{","").replace("}","");
		var ary=jsztPie.split(",");
		for(var i=0;i<ary.length;i++){
			var ary1=ary[i].split("=");
			jszt_data.push(parseFloat(ary1[1]));
			jszt_pei.push([$.trim(ary1[0]),parseFloat(ary1[1])]);
		}
	...
	var xmlbColumn=$("#jstzColumn").val().replace("{","").replace("}","");
	...
	//建设状态数据求和
	var sumJszt=0;
	for(var i=0;i<jszt_data.length;i++){
		sumJszt+=jszt_data[i];
	}
	if(sumJszt!=0){
		pieChart();
