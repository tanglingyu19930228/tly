...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="tswlx"><i class="t_name">铁丝网类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                	<li><a>铁丝网类型</a><i class="icons icons-up"></i></li>
    ...
    <tbody id="tbody">
    	<c:forEach items="${pageBean.recordList }" var="item">
			<tr>
				...
				<td>${fn:escapeXml(item.tswlx) }</td><!-- 铁丝网类型-->
				<td>${fn:escapeXml(item.lzmgs) }</td><!-- 拦阻门个数-->
				<c:if test="${partTag eq '3' or partTag eq '4'}">
...
<script>
	...
	function ajaxData(){
		...
		var tswlxTag = false;//铁丝网类型是否显示标识
		...
		var tswlx = null;
		if($("#tswlx").find(".t_name").text() !="铁丝网类型"){
			if(tswlxTag){
				tswlx = $("#tswlx").find(".t_name").text();
			}
		}
		/* 建设状态 */
		var jszt=null;
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,"dxlb":dxlb,
			"jsxz":jsxz,"tznd":tznd,"tswlx":tswlx,"offiset":$("#offiset").val(),"partTag":partTag ,"jszt":jszt,"syzt":syzt};
		$.ajax({
				...
				if(data!=null){
					for(var i=0;i<data.recordList.length;i++){
						...
						$("#tbody").append(
				            ...
				            "<td>"+data.recordList[i].tswlx+"</td>"+
				            "<td>"+data.recordList[i].lzmgs+"</td>"+
				            ssjdTd+syztTd+
				            "</tr>"
						);
					}
