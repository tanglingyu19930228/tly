...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="lzzlx"><i class="t_name">拦阻桩类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                	<li><a>拦阻桩类型</a><i class="icons icons-up"></i></li>
    ...
    <tbody id="tbody">
    	<c:forEach items="${pageBean.recordList }" var="item">
			<tr>
				...
				<td>${fn:escapeXml(item.lzzlx) }</td><!-- 拦阻桩类型-->
				<c:if test="${partTag eq '3' or partTag eq '4'}">
...
<script>
	...
	function ajaxData(){
		...
		var lzzlxTag = false;//拦阻桩类型是否显示标识
		...
		var lzzlx = null;
		if($("#lzzlx").find(".t_name").text() !="拦阻桩类型"){
			if(lzzlxTag){
				lzzlx = $("#lzzlx").find(".t_name").text();
			}
		}
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,"dxlb":dxlb,
			"jsxz":jsxz,"tznd":tznd,"lzzlx":lzzlx,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
		$.ajax({
