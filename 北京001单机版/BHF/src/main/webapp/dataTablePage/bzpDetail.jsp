...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="bzplx"><i class="t_name">标志牌类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                    <li><a>标志牌类型</a><i class="icons icons-up"></i></li>
...
<script>
	...
	function ajaxData(){
		...
		var bzplxTag = false;//标志牌类型是否显示标识
		...
		var bzplx = null;
		if($("#bzplx").find(".t_name").text()!="标志牌类型"){
			if(bzplxTag){
				bzplx=$("#bzplx").find(".t_name").text();
			}
		}
		var jszt = null;
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,
			"dxlb":dxlb,"jsxz":jsxz,"tznd":tznd,"bzplx":bzplx,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
		$.ajax({
