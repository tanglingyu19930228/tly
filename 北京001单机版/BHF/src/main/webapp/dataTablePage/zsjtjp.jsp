...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="tjplx"><i class="t_name">停机坪类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                    <li><a>停机坪类型</a><i class="icons icons-up"></i></li>
    ...
    <tbody id="tbody">
    	<c:forEach items="${pageBean.recordList }" var="item">
			<tr>
				...
				<td>${fn:escapeXml(item.tjplx) }</td><!-- 停机坪类型-->
				<c:if test="${partTag eq '3' or partTag eq '4'}">
...
<script>
	...
	function ajaxData(){
		...
		var tjplxTag = false;//停机坪类型是否显示标识
		...
		var tjplx = null;
		if($("#tjplx").find(".t_name").text() !="停机坪类型"){
			if(tjplxTag){
				tjplx = $("#tjplx").find(".t_name").text();
			}
		}
		/* 建设状态 */
		var jszt=null;
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,"dxlb":dxlb,
			"jsxz":jsxz,"tznd":tznd,"tjplx":tjplx,"offiset":$("#offiset").val(),"partTag":partTag ,"jszt":jszt,"syzt":syzt};
		$.ajax({
