...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="xlxz"><i class="t_name">线路性质</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                    <li><a>线路性质</a><i class="icons icons-up"></i></li>
            ...
            <th class="thPosition">
                <span class="typeTitle" id="xllx"><i class="t_name">线路类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                    <li><a>线路类型</a><i class="icons icons-up"></i></li>
    ...
    <tbody id="tbody">
    	<c:forEach items="${pageBean.recordList }" var="item">
	        <tr>
	            ...
	            <td>${fn:escapeXml(item.xlqd) }</td>
	            <td>${fn:escapeXml(item.xlzd) }</td>
	            <td>${fn:escapeXml(item.jsdd)}</td>
	            <td>${fn:escapeXml(item.jsgm) }</td>
	            <td>${fn:escapeXml(item.xlxz)}</td>
	            <td>${fn:escapeXml(item.xllx) }</td>
	            <c:if test="${partTag eq '3' or partTag eq '4'}">
...
<script>
	...
	function ajaxData(){
		...
		var xlxzTag = false;//线路性质是否显示标识
		var xllxTag = false;//线路类型是否显示标识
		...
		var xlxz=null;
		if($("#xlxz").find(".t_name").text()!="线路性质"){ 
			if(xlxzTag){
				xlxz=$("#xlxz").find(".t_name").text();
			}
		}	
		var xllx = null;
		if($("#xllx").find(".t_name").text()!="线路类型"){
			if(xllxTag){
				xllx=$("#xllx").find(".t_name").text();
			}
		}
		var jszt=null;
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,"tznd":tznd,
			"dxlb":dxlb,"jsxz":jsxz,"xlxz":xlxz,"xllx":xllx,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
		$.ajax({
				...
				if(data!=null){
					for(var i=0;i<data.recordList.length;i++){
						...
						$("#tbody").append(
				            ...
				            "<td>"+data.recordList[i].xlqd+"</td>"+
				            "<td>"+data.recordList[i].xlzd+"</td>"+
				            "<td>"+data.recordList[i].jsdd+"</td>"+
				            "<td>"+data.recordList[i].jsgm+"</td>"+
				            "<td>"+data.recordList[i].xlxz+"</td>"+
				            "<td>"+data.recordList[i].xllx+"</td>"+
				            ssjdTd+syztTd+
				            "</tr>"
						);
					}
