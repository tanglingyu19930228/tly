...
<table class="tabList comBorder">
    <thead id="thead">
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="qllx"><i class="t_name">桥梁类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                    <li><a>桥梁类型</a><i class="icons icons-up"></i></li>
    ...
    <tbody id="tbody">
    	<c:forEach items="${pageBean.recordList }" var="item">
			<tr>
				...
				<td>${fn:escapeXml(item.qllx) }</td><!-- 桥梁类型-->
				<td><fmt:formatNumber value="${item.zz}" maxFractionDigits="4"/></td><!-- 载重 -->
				<c:if test="${partTag eq '3' or partTag eq '4'}">
...
<script>
	...
	function ajaxData(){
		...
		var qllx = null;
		if($("#qllx").find(".t_name").text() !="桥梁类型"){
			if(qllxTag){
				qllx = $("#qllx").find(".t_name").text();
			}
		}
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,"dxlb":dxlb,
			"jsxz":jsxz,"tznd":tznd,"qllx":qllx,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
		$.ajax({
				...
				if(data!=null){
					for(var i=0;i<data.recordList.length;i++){
						...
						$("#tbody").append(
				            ...
				            "<td>"+data.recordList[i].qllx+"</td>"+
				            "<td>"+data.recordList[i].zz+"</td>"+
				            ssjdTd+syztTd+
				            "</tr>"
						);
					}
