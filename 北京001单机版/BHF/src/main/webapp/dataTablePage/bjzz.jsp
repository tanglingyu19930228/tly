    ...
    <tbody id="tbody">
    	<c:forEach items="${pageBean.recordList }" var="item">
	        <tr>
	            ...
	            <td>${fn:escapeXml(item.sbpp) }</td>
	            <td>${fn:escapeXml(item.sbxh) }</td>
	            <c:if test="${partTag eq '3' or partTag eq '4'}">
...
<script>
	...
	function ajaxData(){
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,
			"dxlb":dxlb,"jsxz":jsxz,"tznd":tznd,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
		$.ajax({
					...
					for(var i=0;i<data.recordList.length;i++){
						...
						$("#tbody").append(
			                ...
			                "<label for=''></label></div></td>"+
				       		...
				       		"<td><span class='nameTd' title='"+data.recordList[i].xmmc+"'>"+data.recordList[i].xmmc+"</span></td>"+
				            ...
				            /* "<td class=file-name' title='"++"'>"+data.recordList[i].fj+"</td>"+ */
				            ...
				            "<td>"+data.recordList[i].sbpp+"</td>"+
				            "<td>"+data.recordList[i].sbxh+"</td>"+
				            ssjdTd+syztTd+
				            "</tr>"
						);
					}
