...
<table class="tabList comBorder">
	<thead>
		<tr>
			...
			<th>建设规模</th>
			<th class="thPosition">
				<span class="typeTitle" id="dllb">
					<i class="t_name">道路类别</i><i class="icons icons-down"></i>
					<ul class="typeItem">
	                    <li><a>道路类别</a><i class="icons icons-up"></i></li>
			...
			<th class="thPosition">
				<span class="typeTitle" id="dllx">
					<i class="t_name">道路类型</i><i class="icons icons-down"></i>
					<ul class="typeItem">
	                    <li><a>道路类型</a><i class="icons icons-up"></i></li>
			...
			<th class="thPosition">
				<span class="typeTitle" id="dldj">
					<i class="t_name">道路等级</i><i class="icons icons-down"></i>
					<ul class="typeItem">
	                    <li><a>道路等级</a><i class="icons icons-up"></i></li>
			...
			<th class="thPosition">
				<span class="typeTitle" id="ljlx">
					<i class="t_name">路基类型</i><i class="icons icons-down"></i>
					<ul class="typeItem">
	                    <li><a>路基类型</a><i class="icons icons-up"></i></li>
			...
			<th class="thPosition">
				<span class="typeTitle" id="lmlx">
					<i class="t_name">路面类型</i><i class="icons icons-down"></i>
					<ul class="typeItem">
	                    <li><a>路面类型</a><i class="icons icons-up"></i></li>
	...
	<tbody id="tbody">
		<c:forEach items="${pageBean.recordList }" var="item">
			<tr>
				...
				<td>${fn:escapeXml(item.jd) }</td>	<!-- 经度 --> 
				<td>${fn:escapeXml(item.wd) }</td>	<!-- 纬度 --> 
				...
				<td id="beizhu" title="${fn:escapeXml(item.bz) }">${fn:escapeXml(item.bz) }</td>	<!-- 备注-->
				<td>${fn:escapeXml(item.jsdd) }</td><!-- 建设地点 -->
				<td><fmt:formatNumber value="${item.jsgm}" maxFractionDigits="4"/></td><!-- 建设规模-->
				<td>${fn:escapeXml(item.dllb) }</td><!-- 道路类别 -->
				<td>${fn:escapeXml(item.dllx) }</td><!-- 道路类型 -->
				<td>${fn:escapeXml(item.dldj) }</td><!-- 道路等级 -->
				<td>${fn:escapeXml(item.ljlx) }</td><!-- 路基类型 -->
				<td>${fn:escapeXml(item.lmlx) }</td><!-- 路面类型 -->
				<c:if test="${partTag eq '3' or partTag eq '4'}">
...
function ajaxData(){
	...
	var dllbTag = false;//道路类别是否显示标识
	var dllxTag = false;//道路类型是否显示标识
	var dldjTag = false;//道路等级是否显示标识
	var ljlxTag = false;//路基类型是否显示标识
	var lmlxTag = false;//路面类型是否显示标识
	...
	var dllb = null;
	if($("#dllb").find(".t_name").text() !="道路类别"){
		if(dllbTag){
			dllb = $("#dllb").find(".t_name").text();
		}
	}
	var dllx = null;
	if($("#dllx").find(".t_name").text() !="道路类型"){
		if(dllxTag){
			dllx = $("#dllx").find(".t_name").text();
		}
	}
	var dldj = null;
	if($("#dldj").find(".t_name").text() !="道路等级"){
		if(dldjTag){
			dldj = $("#dldj").find(".t_name").text();
		}
	}
	var ljlx = null;
	if($("#ljlx").find(".t_name").text() !="路基类型"){
		if(ljlxTag){
			ljlx = $("#ljlx").find(".t_name").text();
		}
	}
	var lmlx = null;
	if($("#lmlx").find(".t_name").text() !="路面类型"){
		if(lmlxTag){
			lmlx = $("#lmlx").find(".t_name").text();
		}
	}
	var jszt = null;
	...
	var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,"dxlb":dxlb,
		"jsxz":jsxz,"tznd":tznd,"dllb":dllb,"dllx":dllx,"dldj":dldj,"ljlx":ljlx,"lmlx":lmlx,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
	$.ajax({
			...
			if(data!=null){
				for(var i=0;i<data.recordList.length;i++){
					...
					$("#tbody").append(
			            ...
			            "<td id='beizhu' title='"+data.recordList[i].bz+"'>"+data.recordList[i].bz+"</td>"+
			            "<td>"+data.recordList[i].jsdd+"</td>"+
			            "<td>"+data.recordList[i].jsgm+"</td>"+
			            "<td>"+data.recordList[i].dllb+"</td>"+
			            "<td>"+data.recordList[i].dllx+"</td>"+
			            "<td>"+data.recordList[i].dldj+"</td>"+
			            "<td>"+data.recordList[i].ljlx+"</td>"+
			            "<td>"+data.recordList[i].lmlx+"</td>"+
			            ssjdTd+syztTd+
			            "</tr>"
					);
				}
