...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th>建设规模</th>
            <th class="thPosition">
                <span class="typeTitle" id="zqflx"><i class="t_name">执勤房类型</i><i class="icons icons-down"></i>
	                <ul class="typeItem">
	                    <li><a>执勤房类型</a><i class="icons icons-up"></i></li>
...
<script>
	...
	function ajaxData(){
		...
		var zqflxTag = false;//执勤房类型是否显示标识
		...
		var zqflx = null;
		if($("#zqflx").find(".t_name").text()!="执勤房类型"){
			if(zqflxTag){
				zqflx=$("#zqflx").find(".t_name").text();
			}
		}
		var jszt = null;
		...
		var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,
			"dxlb":dxlb,"jsxz":jsxz,"tznd":tznd,"zqflx":zqflx,"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
		$.ajax({
