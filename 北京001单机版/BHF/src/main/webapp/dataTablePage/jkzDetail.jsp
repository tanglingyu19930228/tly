...
<table class="tabList comBorder">
    <thead>
        <tr>
            ...
            <th class="thPosition">
                <span class="typeTitle" id="jszt"><i class="t_name">建设状态</i><i class="icons icons-down"></i>
                <ul class="typeItem">
    	...
    	<c:forEach items="${pageBean.recordList }" var="item">
	        <tr>
	            ...
	            <td onclick="showJkz_zl(this, '', '显控终端');" style='cursor:pointer;'>${fn:escapeXml(item.xkzdgs) }</td>
	            <td onclick="showJkz_zl(this,'移动式', '视频前端');" style='cursor:pointer;'>${fn:escapeXml(item.ydqdgs) }</td>
	            <td onclick="showJkz_zl(this,'固定式', '视频前端');" style='cursor:pointer;'>${fn:escapeXml(item.gdqdgs) }</td>
	            <c:if test="${partTag eq '3' or partTag eq '4'}">
	...
	function ajaxData(){
		...
		var syzt=null;
			...
			var condition = {"szsf":'${szsf}',"xmlb":'${xmlb}',"xmzl":'${xmzl}',"sslx":sslx,"bjfx":bjfx,
				...
				"offiset":$("#offiset").val(),"partTag":partTag,"jszt":jszt,"syzt":syzt};
			$.ajax({
				...
				success:function(data){
					...
					if(data!=null){
						for(var i=0;i<data.recordList.length;i++){
							...
							$("#tbody").append(
					            ...
					            "<td class='xkzdFn' style='cursor:pointer;'>"+data.recordList[i].xkzdgs+"</td>"+					            
					            "<td class='spqdYdsFn' style='cursor:pointer;'>"+data.recordList[i].ydqdgs+"</td>"+					            
					            "<td class='spqdGdsFn' style='cursor:pointer;'>"+data.recordList[i].gdqdgs+"</td>"+					            
					            ssjdTd+syztTd+
					            "</tr>"
							);
						}
					...
					$(".xkzdFn").click(function(){
						showJkz_zl(this, '', '显控终端');
					});
					$(".spqdYdsFn").click(function(){
						showJkz_zl(this, '移动式', '视频前端');
					});
					$(".spqdGdsFn").click(function(){
						showJkz_zl(this, '固定式', '视频前端');
					});
				}
			});
	}
	
	function showJkz_zl(obj, sblx, xmzl){
		
		var jkz_id = $(obj).parent("tr").find("td:eq(0) input[type='checkbox']").val();
		$("#jkz_zlQueryDiv").empty();
		$("#jkz_zlQueryDiv").load("xmzl/showJkz_zl",{jkz_id:jkz_id, xmzl:xmzl, sblx:sblx, partTag:partTag},function(){
			$("#jkz_zlDlg").parent().css("top","20%");
			$("#jkz_zlDlg").show().dialog("open");
		});
	}
</script>